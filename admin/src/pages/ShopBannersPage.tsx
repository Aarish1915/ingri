...
import { Plus, Edit2, Trash2, Eye, EyeOff, ChevronUp, ChevronDown, Upload, Image, Loader2, X, GripVertical } from 'lucide-react';
import { adminFetch } from '../App';

type ShopBanner = {
  id: string; _id?: string; title: string; desktopImage: string;
  mobileImage: string; image: string; description: string;
  active: boolean; displayOrder: number; startsAt?: string; endsAt?: string;
};

function getId(b: ShopBanner) { return b.id || b._id || ''; }
...
const emptyForm: FormData = {
  title: '', desktopImage: '', mobileImage: '', description: '',
...
export default function ShopBannersPage() {
  ...
  const load = async () => {
    try {
      const res = await adminFetch('/admin/shop-banners');
      if (res.ok) { const data = await res.json(); setBanners((data.banners || []).sort((a: ShopBanner, b: ShopBanner) => (a.displayOrder || 0) - (b.displayOrder || 0))); }
    } catch (e) { console.error('Failed to load shop banners:', e); }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const bid = editing ? getId(editing) : null;
      const payload = { ...formData, image: formData.desktopImage };
      const url = bid ? `/admin/shop-banners/${bid}` : '/admin/shop-banners';
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Delete this shop banner?')) return;
    try { const res = await adminFetch(`/admin/shop-banners/${id}`, { method: 'DELETE' }); if (res.ok) load(); }
    catch (e) { console.error('Failed to delete:', e); }
  };

  const handleEdit = (b: ShopBanner) => {
    setEditing(b);
    setFormData({ title: b.title || '', desktopImage: b.desktopImage || b.image || '', mobileImage: b.mobileImage || '', description: b.description || '', active: b.active, displayOrder: b.displayOrder || 0, startsAt: b.startsAt || '', endsAt: b.endsAt || '' });
    setShowForm(true);
  };

  const toggleActive = async (b: ShopBanner) => {
    try { await adminFetch(`/admin/shop-banners/${getId(b)}`, { method: 'PATCH', body: JSON.stringify({ active: !b.active }) }); load(); }
  ...
  const reorder = async (index: number, direction: -1 | 1) => {
    ...
    try {
      await Promise.all([
        adminFetch(`/admin/shop-banners/${getId(a)}`, { method: 'PATCH', body: JSON.stringify({ displayOrder: b.displayOrder ?? swapIdx }) }),
        adminFetch(`/admin/shop-banners/${getId(b)}`, { method: 'PATCH', body: JSON.stringify({ displayOrder: a.displayOrder ?? index }) }),
      ]);
      load();
  ...
  const getPreview = (b: ShopBanner) => b.desktopImage || b.image || b.mobileImage || '';
  ...
  return (
    <div className="max-w-5xl">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-lg font-semibold text-[#1A4547]">Shop Banners</h1>
          <p className="text-xs text-gray-400 mt-0.5">Manage shop page hero carousel</p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100">
              <h2 className="text-sm font-semibold text-gray-800">{editing ? 'Edit Shop Banner' : 'New Shop Banner'}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-gray-100">
                ...
                <button type="submit" disabled={saving || uploadingDesktop || uploadingMobile}
                  ...
                  {saving ? <><Loader2 size={12} className="animate-spin" /> Saving...</> : editing ? 'Update' : 'Create'}
                </button>
              </div>
            </form>
          </div>
      ...
      {banners.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-xl border border-gray-100">
          ...
          <p className="text-gray-400 text-sm">No shop banners yet</p>
        </div>
      ) : (
        <div className="space-y-2">
          {banners.map((banner, idx) => (
            <div key={getId(banner)} className={`flex items-center gap-3 p-3 bg-white rounded-xl border border-gray-100 ${!banner.active ? 'opacity-50' : ''}`}>
              ...
              <div className="flex gap-2 flex-shrink-0">
                <div className="w-24 h-14 rounded-lg overflow-hidden border border-gray-100 bg-gray-50">
                  <img src={getPreview(banner)} alt={banner.title} className="w-full h-full object-cover" />
                </div>
                {banner.mobileImage && (
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                ...
                <button onClick={() => handleDelete(getId(banner))} className="p-1.5 rounded-lg text-gray-300 hover:text-red-500 hover:bg-red-50"><Trash2 size={15} /></button>
              </div>
            </div>
          ))}
        </div>
